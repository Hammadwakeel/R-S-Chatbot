...
import { 
  ...
  Archive, 
  Loader2,
  Terminal // Added terminal icon for logs
} from 'lucide-react'
...
export default function AdminDashboard() {
  ...
  const handleBulkFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setBulkStatusMsg("Initializing bulk stream...")
    setBulkLog([]) // Clear logs

    await api.ingestion.uploadBulk(file, {
      onStatusUpdate: (eventData) => {
        // --- LOG EVERYTHING ---
        // This ensures you see movement even if specific events don't match exactly
        const time = new Date().toLocaleTimeString().split(" ")[0];
        let logMsg = "";

        if (eventData.event === "initialized") logMsg = "Job initialized.";
        else if (eventData.event === "upload_complete") logMsg = "Source uploaded to storage.";
        else if (eventData.event === "batch_start") logMsg = `Found ${eventData.total_files} files to process.`;
        else if (eventData.event === "processing_started") logMsg = "Processing pipeline started...";
        else if (eventData.event === "file_finished") logMsg = `Finished: ${eventData.filename}`;
        else if (eventData.event === "batch_completed") logMsg = `Batch Done! Success: ${eventData.stats?.successful_files || 0}`;
        else if (eventData.event === "error") logMsg = `Error: ${eventData.error}`;
        else logMsg = `Event: ${eventData.event}`; // Fallback for unknown events

        if (logMsg) {
          setBulkLog(prev => [`[${time}] ${logMsg}`, ...prev].slice(0, 10)) // Keep last 10 logs
        }
        setBulkStatusMsg(logMsg);
      },
      onError: (msg) => {
        setErrorMessage(msg)
      ...
      onComplete: (reportUrl) => {
        setBulkStatusMsg("Batch Completed!")
        setSuccessMessage("Batch processing finished successfully.")
        setIsBulkUploading(false)
        fetchRemoteFiles()
        // Automatically open the master report
            ...
            <div className="space-y-4">
              ...
              {isBulkUploading && (
                <div className="bg-zinc-950 p-3 rounded-sm text-xs font-mono text-emerald-400 border border-zinc-800 shadow-inner">
                  ...
                  <div className="space-y-1 h-24 overflow-y-auto scrollbar-thin scrollbar-thumb-zinc-700">
          ...
          <div className="p-6">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {remoteUploadedPdfs.map((name) => (
                <div key={name} className="flex items-center justify-between p-3 bg-zinc-50 dark:bg-zinc-900/50 rounded-sm border">
                  <div className="truncate text-sm font-medium w-2/3">{name}</div>
